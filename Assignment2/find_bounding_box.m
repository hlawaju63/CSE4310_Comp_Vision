...
function [top, bottom, left, right] = find_bounding_box(img_name)
...
frame_filename1 = make_frame_name(sequence_name, frame-15);
frame_filename2 = make_frame_name(sequence_name, frame+15);



%reading the gray_frame 5 frame ahead and behind
frame37 = read_gray(frame_filename1);
frame67 = read_gray(frame_filename2);

%displaying gray_images
%figure(2); imshow(frame37,[]);
%figure(3); imshow(frame67,[]);


diff1 = abs(frame37 - frame52);
%figure(4); 
imshow(diff1, []);
diff2 = abs(frame52 - frame67);
%figure(5); 
imshow(diff2, []);
...
counters = zeros(1, number);
    ...
    original_image = double(imread(img_name));
    % for color images, range must be [0 1] for imshow to work
    figure(11);imshow(original_image / 255);
    
    result_image = original_image; %make a copy
  
    color = [255, 255, 0];
    
    result = draw_rectangle(result_image, color, top, bottom, left, right);
    figure(12); imshow(result/255)
